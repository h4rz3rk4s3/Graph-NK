...
def iter_bulk_records(
    ...
    *,
    ...
    progress_cb (optional, default off — existing callers are unaffected):
        called every `progress_every_lines` lines with the number of COMPRESSED
        bytes consumed from the file so far. Compared against
        os.path.getsize(path) this gives a fraction-complete for the file, which
        is the only cheap way to estimate remaining time: the uncompressed
        record count is unknown until the file has been read.

        Caveat: the value is the position of the underlying binary reader, so
        buffering makes it run slightly AHEAD of what has actually been
        decompressed and yielded. It is monotonic and bounded by the file size,
        which is all an ETA needs — do not use it for exact accounting.
    """
    pending_urn: str | None = None
    with open(path, "rb") as raw:
        with gzip.open(raw, "rt", encoding="utf-8", errors="replace") as fh:
            for line_no, line in enumerate(fh, start=1):
                if progress_cb is not None and progress_every_lines \
                        and line_no % progress_every_lines == 0:
                    progress_cb(raw.tell())
                line = line.strip()
                if not line:
                    continue
                try:
            ...
            if progress_cb is not None:
                progress_cb(raw.tell())   # final position, so callers land on 100%
